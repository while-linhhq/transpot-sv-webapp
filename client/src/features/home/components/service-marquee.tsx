...
export function ServiceMarquee() {
  ...
  return (
    <section className="relative z-20 border-y-2 border-accent bg-brand-yellow py-2">
      ...
      <div className="overflow-hidden">
        <div className="marquee-track flex w-max flex-nowrap items-center" style={trackStyle}>
          {flat.map((item, index) => (
            <span
              key={`${item}-${index}`}
              className="inline-flex shrink-0 items-center gap-4 whitespace-nowrap px-4 text-xs font-extrabold uppercase tracking-wide text-foreground md:text-sm"
            >
